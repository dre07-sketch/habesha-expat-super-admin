    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white/80 dark:bg-slate-900/80 border border-white/40 dark:border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in backdrop-blur-xl ring-1 ring-black/5">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                item.type === ContentType.BUSINESS ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 ring-1 ring-blue-500/20' :
                item.type === ContentType.VIDEO ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 ring-1 ring-purple-500/20' :
                'bg-orange-500/10 text-orange-600 dark:text-orange-400 ring-1 ring-orange-500/20'
              }`}>
                {item.type}
              </span>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                item.status === Status.ACTIVE ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 ring-1 ring-emerald-500/20' : 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 ring-1 ring-yellow-500/20'
              }`}>
                {item.status}
              </span>
            </div>
        ...
        <div className="flex border-b border-slate-100/50 dark:border-white/5 px-6 bg-white/20 dark:bg-white/5">
            ...
            >
                Comments <span className="bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded-full text-[10px]">{item.comments || 4}</span>
            </button>
            <button 
            ...
            >
                Likes <span className="bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded-full text-[10px]">{item.likes || 6}</span>
            </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div onClick={() => setActiveTab('likes')} className="cursor-pointer p-3 bg-white dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5 flex flex-col items-center justify-center gap-1 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors">
                        ...
                        <span className="text-lg font-bold text-slate-800 dark:text-white">{item.likes?.toLocaleString() || '0'}</span>
                    ...
                    <div onClick={() => setActiveTab('comments')} className="cursor-pointer p-3 bg-white dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5 flex flex-col items-center justify-center gap-1 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors">
                        ...
                        <span className="text-lg font-bold text-slate-800 dark:text-white">{item.comments?.toLocaleString() || '0'}</span>
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <span className="text-slate-800 dark:text-white font-medium">{item.date}</span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-white/5 bg-white/50 dark:bg-white/5">
                ...
                {item.type === ContentType.BUSINESS && (
                    <div className="bg-blue-50/50 dark:bg-blue-900/10 p-5 rounded-2xl border border-blue-100 dark:border-blue-500/20">
                        <h3 className="text-xs font-bold text-blue-500 mb-3 uppercase tracking-wider">Business Information</h3>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-slate-800 dark:text-white">{item.location || 'Addis Ababa'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-slate-800 dark:text-white">{item.rating || 'N/A'} <span className="font-normal text-slate-400 text-xs">/ 5.0</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
          ...
          {activeTab === 'comments' && (
            <div className="space-y-4 animate-fade-in">
                {MOCK_COMMENTS.map((comment) => (
                    <div key={comment.id} className="flex gap-4 p-4 rounded-2xl bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-md">
                            {comment.avatar}
                        </div>
                        <div className="flex-1">
                ...
                <div className="p-4 text-center">
                    <button className="text-sm text-blue-500 hover:underline">Load more comments</button>
                </div>
            </div>
          )}
           ...
           {activeTab === 'likes' && (
            <div className="animate-fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {MOCK_LIKES.map((like) => (
                        <div key={like.id} className="flex items-center gap-4 p-3 rounded-2xl bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-blue-200 dark:hover:border-blue-500/30 transition-colors">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-teal-400 flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-md">
                                {like.avatar}
                            </div>
                            <div>
                                <h4 className="font-bold text-slate-800 dark:text-white text-sm">{like.user}</h4>
                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                                    like.role === 'Premium' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
